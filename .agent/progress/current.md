...
- Leave management is unavailable until migration `20260618000500_leave_management.sql` is approved and applied. The web Leave panel shows a "being set up" notice until then. The running API also needs a restart to load the new `leave` router. (Leave migration was applied 2026-06-18.)
- Smart Shifts is unavailable until migration `20260619000600_shifts.sql` is approved and applied. The web Shifts panel shows a "being set up" notice until then; the running API needs a restart to load the `modules/shifts` router.
